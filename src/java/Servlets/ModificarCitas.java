...
public class ModificarCitas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            int Numeroregistro = Integer.parseInt(request.getParameter("registro"));
            String idenPaciente = request.getParameter("idpaciente");
            String idenMedico = request.getParameter("idmedico");
            String fechacita = request.getParameter("Fecha");

            Consultas cons = new Consultas("clinica", "root", "");

            boolean validado = cons.modificarCita(Numeroregistro, idenPaciente, idenMedico, fechacita);
            ...
            if (validado) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Modificar cita</title>");
                ...
                out.println("<div class='Encabezado'><h2>Oficinista <%=nombre%></h2></div>");
                out.println("<div class='Contenido'>");
                out.println("<br><h1>No se pudo modificar la cita: " + Numeroregistro + "</h1><br>");
                out.println("<br><a href='Citas.jsp'><button class='btn btn-secondary'>Volver</button></a>");
                out.println("</div>");
